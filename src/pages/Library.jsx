    ...
    <main className="min-h-screen pb-40">
      <div className="mx-auto max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 border-b border-white/10 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.18em] text-amber/80">Library</div>
            <h1 className="mt-2 font-display text-4xl font-black text-white">Saved creations</h1>
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-blue-50/65">
          ...
          <button
            ...
            className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-[linear-gradient(135deg,#2563eb,#38bdf8)] px-4 text-sm font-black text-white shadow-[0_0_34px_rgba(56,189,248,0.24)] transition hover:brightness-105"
          >
            <Wand2 size={17} />
            New Creation
          </button>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_auto_auto] lg:items-center">
          <label className="flex h-11 items-center gap-3 rounded-md border border-white/10 bg-white/5 px-3 text-blue-50/65">
            <Search size={17} />
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-white outline-none placeholder:text-blue-50/35"
          ...
          <button
            type="button"
            onClick={refresh}
            className="inline-flex h-11 items-center justify-center gap-2 rounded-md border border-white/10 bg-white/5 px-3 text-xs font-black text-blue-50/80 transition hover:border-amber/30 hover:bg-white/10 hover:text-white"
          >
        ...
        <section className="mt-5 grid gap-3">
          {filteredCreations.map((creation) => {
            ...
            return (
              <article key={creation.id} className="grid gap-4 rounded-md border border-white/10 bg-[rgba(9,24,44,0.88)] p-4 shadow-[0_20px_60px_rgba(56,189,248,0.08)] xl:grid-cols-[minmax(0,1fr)_340px]">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-md bg-[linear-gradient(135deg,#2563eb,#38bdf8)] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-white">
                      {creation.kind}
                    </span>
                    <span className="rounded-md border border-amber/30 bg-amber/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-amber">
                      {creation.style}
                    </span>
                  </div>
                  <h2 className="mt-3 truncate font-display text-2xl font-black text-white">{creation.title}</h2>
                  <p className="mt-2 text-sm font-semibold leading-6 text-blue-50/70">{creation.prompt}</p>
                </div>
                ...
                <div className="grid gap-3">
                  <div className="flex h-16 items-end gap-1 rounded-md border border-white/10 bg-white/5 p-2">
                    {(creation.result?.previewPeaks?.length ? creation.result.previewPeaks : Array.from({ length: 40 }, (_, index) => 0.12 + ((index * 13) % 30) / 60)).slice(0, 44).map((peak, index) => (
                      <span key={`${creation.id}-${index}`} className="flex-1 rounded-t bg-[linear-gradient(180deg,rgba(255,204,102,0.92),rgba(56,189,248,0.78))]" style={{ height: `${Math.round(18 + peak * 78)}%` }} />
                    ))}
                  </div>
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    <button
                      ...
                      className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-white px-3 text-xs font-black text-[#08101b] transition hover:bg-amber"
                    >
                    ...
                    <button
                      ...
                      className="grid h-10 w-10 place-items-center rounded-md border border-white/10 bg-white/5 text-blue-50/80 transition hover:border-sky/30 hover:bg-white/10 hover:text-white"
          ...
          {!filteredCreations.length && (
            <div className="rounded-md border border-white/10 bg-[rgba(9,24,44,0.88)] p-8 text-center shadow-[0_20px_60px_rgba(56,189,248,0.08)]">
              <h2 className="font-display text-2xl font-black text-white">No creations found</h2>
              <p className="mx-auto mt-2 max-w-md text-sm font-semibold leading-6 text-blue-50/65">
                Generate a new song, instrumental, or remix and it will be saved with its original prompt.
              </p>
              <button
                type="button"
                onClick={() => navigate('/create')}
                className="mt-5 inline-flex h-11 items-center justify-center gap-2 rounded-md bg-[linear-gradient(135deg,#2563eb,#38bdf8)] px-4 text-sm font-black text-white shadow-[0_0_34px_rgba(56,189,248,0.24)] transition hover:brightness-105"
              >
                <Wand2 size={17} />
                Create Audio
              </button>
